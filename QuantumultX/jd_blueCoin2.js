...
let coinToBeans2 = $.getdata('coinToBeans2') || 0; //兑换多少数量的京豆（20或者1000），0表示不兑换，默认不兑换京豆，如需兑换把0改成20或者1000，或者'商品名称'(商品名称放到单引号内)即可
    ...
    if (cookie) {
      ...
      if ($.isNode()) {
        if (process.env.MARKET_COIN_TO_BEANS) {
          coinToBeans2 = process.env.MARKET_COIN_TO_BEANS;
        }
      }
      try {
        if (`${coinToBeans2}` !== '0') {
          ...
          console.log('查询到您设置的是不兑换京豆选项，现在为您跳过兑换京豆。如需兑换，请去BoxJs设置或者修改脚本coinToBeans2或设置环境变量MARKET_COIN_TO_BEANS\n')
        }
        await msgShow();
      } catch (e) {
        $.logErr(e)
  ...
  if (prizeList && prizeList.length) {
    if (`${coinToBeans2}` === '1000') {
      ...
      for (let index = 0; index < prizeList.length; index ++) {
        if (prizeList[index].title.indexOf(coinToBeans2) > -1) {
      ...
      if (prizeId) {
        if (prizeList[i].inStock === 506 || prizeList[i].inStock === -1) {
          console.log(`失败，您输入设置的${coinToBeans2}领光了，请明天再来`);
          $.beanerr = `失败，您输入设置的${coinToBeans2}领光了，请明天再来`;
          return ;
        }
        ...
        console.log(`奖品兑换列表[${coinToBeans2}]已下架，请检查活动页面是否存在此商品，如存在请检查您的输入是否正确`);
        $.beanerr = `奖品兑换列表[${coinToBeans2}]已下架`;
      }
    }
  }
}
          ...
          if (safeGet(data)) {
            ...
            if ($.data.data.bizCode === 0) {
              if (`${coinToBeans2}` === '1000') {
                ...
                $.beanscount ++;
                console.log(`【京东账号${$.index}】${$.nickName || $.UserName} 第${$.data.data.result.exchangeNum}次换${$.title}成功`)
                if ($.data.data.result.exchangeNum === 20 || $.beanscount === coinToBeans2 || $.data.data.result.blue < 500) return;
              } else {
                $.beanscount ++;
                console.log(`【京东账号${$.index}】${$.nickName || $.UserName} 第${$.data.data.result.exchangeNum}次换${$.title}成功`)
                if ($.beanscount === 1) return;
...
//通知
function msgShow() {
  // $.msg($.name, ``, `【京东账号${$.index}】${$.nickName}\n【收取蓝币】${$.coincount ? `${$.coincount}个` : $.coinerr }${coinToBeans2 ? `\n【兑换京豆】${ $.beanscount ? `${$.beanscount}个` : $.beanerr}` : ""}`);
  return new Promise(async resolve => {
    $.log(`\n【京东账号${$.index}】${$.nickName || $.UserName}\n${coinToBeans2 ? `【兑换${$.title}】${$.beanscount ? `成功` : $.beanerr}` : "您设置的是不兑换奖品"}\n`);
    if ($.isNode() && process.env.MARKET_REWARD_NOTIFY) {
      $.ctrTemp = `${process.env.MARKET_REWARD_NOTIFY}` === 'false';
    } else if ($.getdata('jdSuperMarketRewardNotify2')) {
      $.ctrTemp = $.getdata('jdSuperMarketRewardNotify2') === 'false';
    } else {
      $.ctrTemp = `${jdNotify}` === 'false';
    }
    //默认只在兑换奖品成功后弹窗提醒。情况情况加，只打印日志，不弹窗
    if ($.beanscount && $.ctrTemp) {
      $.msg($.name, ``, `【京东账号${$.index}】${$.nickName || $.UserName}\n${coinToBeans2 ? `【兑换${$.title}】${ $.beanscount ? `成功，数量：${$.beanscount}个` : $.beanerr}` : "您设置的是不兑换奖品"}`);
      allMessage += `【京东账号${$.index}】${$.nickName || $.UserName}\n${coinToBeans2 ? `【兑换${$.title}】${$.beanscount ? `成功，数量：${$.beanscount}个` : $.beanerr}` : "您设置的是不兑换奖品"}${$.index !== cookiesArr.length ? '\n\n' : ''}`
      // if ($.isNode()) {
